...
public class BuildingAgentFindCustomersBehaviour extends TickerBehaviour
{
	...
	public BuildingAgentFindCustomersBehaviour(DFAgentDescription template, BuildingAgent a, long period)
	{
		super(a, period);
		this.building = a;
			...
			if (result.length > 0) 
			{
				for (int i = 0; i < result.length; i++)
				{
					...
					if (!this.customers.contains(c))
					{
						...
						// 
						building.addBehaviour(new BuildingAgentSubscrInitiator((BuildingAgent)building, subscribe));
						//
					}
				}
			}
	 ...
	 * Request message
	 */
	protected ACLMessage createRequestMessage(BuildingAgent sender, AID receiver) 
	{
		Date date = new Date();
