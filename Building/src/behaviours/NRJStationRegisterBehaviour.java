...
import subscription.NRJStationSubscrInitiator;
import agents.NRJStationAgent;

/**
 * Find service in yellow pages
 */
public class NRJStationRegisterBehaviour extends TickerBehaviour 
{
	private static final long serialVersionUID = 3384651319762649616L;
	//
	protected DFAgentDescription template;
	private NRJStationAgent subscriber;
	//
	public NRJStationRegisterBehaviour(DFAgentDescription template, NRJStationAgent a, long period)
	{
		super(a, period);
	...
	protected void onTick() 
	{
		...
		try 
		{
			...
			if (result.length > 0) 
			{
				...
				// 
				subscriber.addBehaviour(new NRJStationSubscrInitiator((NRJStationAgent)subscriber, subscribe));
				//
	 ...
	 * Request message
	 */
	protected ACLMessage createRequestMessage(NRJStationAgent sender, AID receiver) 
	{
		Date date = new Date();
