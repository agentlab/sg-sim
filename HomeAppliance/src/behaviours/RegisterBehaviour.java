...
public class RegisterBehaviour extends TickerBehaviour 
{
	...
	private SubscriberAgent subscriber;
	//
	...
	protected void onTick() 
	{
		...
		try 
		{
			...
			if (result.length > 0) 
			{
				...
				// 
				subscriber.addBehaviour(new SubscrInitiator((SubscriberAgent)subscriber, subscribe));
				//
	 ...
	 * Request message
	 */
	protected ACLMessage createRequestMessage(SubscriberAgent sender, AID receiver) 
	{
		Date date = new Date();
