...
import subscription.NakopitelSubscrInitiator;
import agents.NakopitelEE;

/**
 * Find service in yellow pages
 */
public class NakopitelRegisterBehaviour extends TickerBehaviour 
{
	private static final long serialVersionUID = 3384651319762649616L;
	//
	protected DFAgentDescription template;
	private NakopitelEE subscriber;
	//
	public NakopitelRegisterBehaviour(DFAgentDescription template, NakopitelEE a, long period)
	{
		super(a, period);
	...
	protected void onTick() 
	{
		...
		try 
		{
			...
			if (result.length > 0) 
			{
				...
				// 
				subscriber.addBehaviour(new NakopitelSubscrInitiator((NakopitelEE)subscriber, subscribe));
				//
	 ...
	 * Request message
	 */
	protected ACLMessage createRequestMessage(NakopitelEE sender, AID receiver) 
	{
		Date date = new Date();
