...
public class RequestHPSControllerParamsBehaviour extends OneShotBehaviour {
	...
	public RequestHPSControllerParamsBehaviour(PowerProducerAgent producer) {
		super();
	...
	public void action() {
		...
		System.out.println(producer.getLocalName()+": requesting parameters from HPScontroller "+producer.getControllerDescriptor().getHpsId().getLocalName());
		// Fill the REQUEST message
		ACLMessage msg = new ACLMessage(ACLMessage.REQUEST);
		msg.addReceiver(producer.getControllerDescriptor().getHpsId());	
		...
		msg.setContent("HPScontroller-params-request");
		...
		producer.addBehaviour(new AchieveREInitiator(producer, msg) {
			...
			protected void handleInform(ACLMessage inform) {						
				...
				try {
					...
					if(pr instanceof HPSDescriptor){
						((PowerProducerAgent)myAgent).setControllerDescriptor((HPSDescriptor)pr);
			...
			protected void handleAllResultNotifications(Vector notifications) {
				if (notifications.size() == 0) {
					...
					System.out.println("Timeout expired: missing response from HPS controller");
				}
			}
		} );
	}
