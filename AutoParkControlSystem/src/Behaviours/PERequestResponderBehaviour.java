...
public class PERequestResponderBehaviour extends AchieveREResponder {
	...
	public PERequestResponderBehaviour(Agent a, MessageTemplate mt) {
		super(a, mt);
	...
	protected ACLMessage prepareResponse(ACLMessage msg) throws NotUnderstoodException, RefuseException {
		...
		try {
			...
			if(action instanceof SendPlan) {
				System.out.println("AgentForPE Received SendPlan! All right!");
				agreed = true;
			} else if (action instanceof SendAction) {
				System.out.println("AgentForPE Received SendAction! What are you doing? >(");
				agreed = false;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	protected void performSendPlan(ACLMessage msg){
		...
		myAgent.addBehaviour(new PETickerBehaviour(myAgent, msg));
	}
