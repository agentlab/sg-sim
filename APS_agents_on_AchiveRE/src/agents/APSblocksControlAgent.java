...
public class APSblocksControlAgent extends Agent {

	/**
	 * 
	 */
	
	private Vector<APSblockDescriptor> blocks = new Vector<APSblockDescriptor>(); 		// параметры контролируемых блоков
	...
	private APSDescriptor myParams=new APSDescriptor();									// дескриптор параметров APS
	...
	protected void setup(){
		...
		System.out.println("Hello, APS blocks Control agent "+getAID().getLocalName()+" is started");
		myParams.setAPSId(this.getAID());
	...
	public void addBlock(APSblockDescriptor APSBlock){
		this.blocks.add(APSBlock);
		return;
	}
	...
	public Vector<APSblockDescriptor> getBlocks(){
		return this.blocks;
	}
	public void setBlocks(Vector<APSblockDescriptor> list){
		this.blocks=list;
	}
	...
	public APSDescriptor getMyParams() {
		return myParams;
	}

	public void setMyParams(APSDescriptor myParams) {
	...
	public int getBlockIndex(AID blockID){
		int result=-1;
		for(APSblockDescriptor d:this.blocks){
	...
	protected void takeDown(){
		...
		System.out.println("The APS blocks control agent "+getAID().getLocalName()+" is terminated");
	}
}
