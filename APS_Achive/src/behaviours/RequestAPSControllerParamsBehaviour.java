...
public class RequestAPSControllerParamsBehaviour extends OneShotBehaviour {
	...
	public RequestAPSControllerParamsBehaviour(PowerProducerAgent producer) {
		super();
	...
	public void action() {
		...
		System.out.println(producer.getLocalName()+": requesting parameters from APScontroller "+producer.getControllerDescriptor().getAPSId().getLocalName());
		// Fill the REQUEST message
		ACLMessage msg = new ACLMessage(ACLMessage.REQUEST);
		msg.addReceiver(producer.getControllerDescriptor().getAPSId());	
		...
		msg.setContent("APScontroller-params-request");
		...
		producer.addBehaviour(new AchieveREInitiator(producer, msg) {
			...
			protected void handleInform(ACLMessage inform) {						
				...
				try {
					...
					if(pr instanceof APSDescriptor){
						((PowerProducerAgent)myAgent).setControllerDescriptor((APSDescriptor)pr);
						...
						((PowerProducerAgent)myAgent).seAPSSubMngr(new PowerPlansSubscriptionManager((PowerProducerAgent) myAgent));
						...
						SubscriptionResponder ppSubResponder = new SubscriptionResponder((PowerProducerAgent)myAgent, ppMT, ((PowerProducerAgent)myAgent).geAPSSubMngr());
			...
			protected void handleAllResultNotifications(Vector notifications) {
				if (notifications.size() == 0) {
					...
					System.out.println("Timeout expired: missing response from APS controller");
				}
			}
		} );
	}
