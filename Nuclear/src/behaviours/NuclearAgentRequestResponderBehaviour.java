...
public class NuclearAgentRequestResponderBehaviour extends AchieveREResponder {
	private static final long serialVersionUID = 1L;

	private NuclearPowerPlantAgentBehaviour Nuclear;

	public NuclearAgentRequestResponderBehaviour(NuclearPowerPlantAgentBehaviour wt, Agent a, MessageTemplate mt) {
		super(a, mt);
		Nuclear = wt;
	}
	...
	protected ACLMessage prepareResultNotification(ACLMessage msg, ACLMessage response) throws FailureException {
		...
		try {
			...
			if(action instanceof AssignPowerRequest) {
				...
				if(Nuclear.setAssignedPower(apr.getAssignedPower())) {
			...
			if (action instanceof BeginPowerTransitionRequest) {
				...
				if (bptr.getAssignedPowerIsValid()) {
					...
					if(Nuclear.setAssignedPower(bptr.getAssignedPower())) {
						System.out.println("Check passed, power assigned");
					} else {
						System.out.println("Out of bounds, power not assigned");
						isOk = false;
