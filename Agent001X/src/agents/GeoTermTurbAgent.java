...
public class GeoTermTurbAgent extends Agent {
	...
	protected final String name = "GeoTermTurbine"; // agent service name
	...
	private Ontology ontology = GeoTermTurbOntology.getInstance();
	private int GeoTermTemp=12;
	...
	public void setup() { 
		...
		sd.addOntologies("GeoTerm_Turbine_Ontology");
		dfd.addServices(sd);
		
		ServiceDescription sd2 = new ServiceDescription();
		sd2.setType(type2); // your agent type
		sd2.setName(name); // your agent name
		sd2.addOntologies("GeoTerm_Turbine_Ontology");
		dfd.addServices(sd2);
		...
		try {
			...
			if (this.GeoTermTemp==0) ;
			else System.out.println("Shhhhh...");
			addBehaviour(dfSubscriptionResponder);
			
			this.addBehaviour(new GeoTermTurbBehaviour(this, GeoTermTemp, airDensity, length, bLimit, subManager, dfSubscriptionResponder));
		} catch(Exception e) {
