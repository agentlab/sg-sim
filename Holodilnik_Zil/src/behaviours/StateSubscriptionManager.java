...
public class StateSubscriptionManager implements SubscriptionManager {
	...
	protected sg_sim.Holodilnik_Zil myAgent;
	protected HolodilnikBehaviour holodilnik;
	 ...
	 * @param agent
	 */
	public StateSubscriptionManager(Holodilnik_Zil agent) {
		myAgent = agent;
	...
	public boolean register(Subscription sub) throws RefuseException, NotUnderstoodException {
		...
		if(subscriptions.size() == 1)
			myAgent.addBehaviour(new HolodilnikStateNotificationBehaviour(myAgent, this));
		return true;
	}
	...
	void handleChange(SendMessage msg, String type) {
		for (Subscription sub : subscriptions) {
			if(sub.getMessage().getConversationId().charAt(0) == '0' && type == "TEMPERATURE") {
				notify(sub, msg);
			}
	...
	public void setHolodilnik(HolodilnikBehaviour wt) {
		this.holodilnik = wt;
	}
	
	public HolodilnikBehaviour holodilnik() {
		return this.holodilnik;
	}
}