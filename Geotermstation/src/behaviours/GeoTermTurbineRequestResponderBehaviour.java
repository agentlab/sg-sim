...
public class GeoTermTurbineRequestResponderBehaviour extends AchieveREResponder {
	private static final long serialVersionUID = 1L;

	private GeoTermTurbBehaviour GeoTermTurbine;

	public GeoTermTurbineRequestResponderBehaviour(GeoTermTurbBehaviour wt, Agent a, MessageTemplate mt) {
		super(a, mt);
		GeoTermTurbine = wt;
	}
	...
	protected ACLMessage prepareResultNotification(ACLMessage msg, ACLMessage response) throws FailureException {
		...
		try {
			...
			if(action instanceof AssignPowerRequest) {
				...
				if(GeoTermTurbine.setAssignedPower(apr.getAssignedPower())) {
			...
			if (action instanceof BeginPowerTransitionRequest) {
				...
				if (bptr.getAssignedPowerIsValid()) {
					...
					if(GeoTermTurbine.setAssignedPower(bptr.getAssignedPower())) {
						System.out.println("Check passed, power assigned");
					} else {
						System.out.println("Out of bounds, power not assigned");
						isOk = false;
