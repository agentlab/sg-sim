			...
			if(ce instanceof OneHourResult){
				...
				if(checkPlanLength()){
					...
					for(int k=0;k<24;k++){
						...
						((TPPblocksControlAgent)myAgent).getCurDayResult().setPower(sum, k);
					}
					((TPPblocksControlAgent)myAgent).getCurDayResult().setDate(new Date());
					//calling notify method
					((TPPblocksControlAgent)myAgent).getOdrSubMngr().notify((TPPblocksControlAgent) myAgent);
				}
			}
