...
public class TPPblockAgent extends Agent {
	/**
	 * 
	 */
	private static final long serialVersionUID = -7838901911691186799L;
	private TPPblockDescriptor params=new TPPblockDescriptor(null, 100+10*(Math.random()-0.1), 1000+100*(Math.random()-0.1),  BlockState.Passive);
	private Subscription resultSubscription;
	private OneHourResultSubscriptionManager subMngr;
	private TPPblockControlAction currentAction=new TPPblockControlAction();
	private TPPblockControlAction nextHourAction=new TPPblockControlAction();
	...
	protected void setup(){
		...
		System.out.println(String.format("Hello, TPP control agent %s activated", getAID().getLocalName()));
		params.setId(getAID());
		...
		this.addBehaviour(new AchieveREResponder(this, template) {
			...
			protected ACLMessage prepareResponse(ACLMessage request) throws NotUnderstoodException, RefuseException {
				...
				if (request.getContent().equalsIgnoreCase("params-request")) {
					...
					((TPPblockAgent)myAgent).subMngr = new OneHourResultSubscriptionManager((TPPblockAgent)myAgent);
					MessageTemplate mt = SubscriptionResponder.createMessageTemplate(ACLMessage.SUBSCRIBE);
					SubscriptionResponder subResponder = new SubscriptionResponder(myAgent, mt, ((TPPblockAgent)myAgent).subMngr);
			...
			protected ACLMessage prepareResultNotification(ACLMessage request, ACLMessage response){
					...
					try {
						...
						myAgent.addBehaviour(new ControlCommandsSubscriptionBehaviour((TPPblockAgent)myAgent,request.getSender()));
	...
	public TPPblockDescriptor getParams(){
		return this.params;
	}
	protected void takeDown(){
		...
		System.out.println("TPP control agent "+getAID().getLocalName()+" is terminated");
	}
	...
	public TPPblockControlAction getCurrentAction() {
		return currentAction;
	}
	public void setCurrentAction(TPPblockControlAction currentAction) {
		this.currentAction = currentAction;
	}
	public TPPblockControlAction getNextHourAction() {
		return nextHourAction;
	}
	public void setNextHourAction(TPPblockControlAction nextHourAction) {
