...
import subscription.CustomerAgentSubscrInitiator;
import agents.CustomerAgent;

/**
 * Find service in yellow pages
 */
public class CustomerAgentRegisterBehaviour extends TickerBehaviour 
{
	private static final long serialVersionUID = 3384651319762649616L;
	//
	protected DFAgentDescription template;
	private CustomerAgent subscriber;
	//
	public CustomerAgentRegisterBehaviour(DFAgentDescription template, CustomerAgent a, long period)
	{
		super(a, period);
	...
	protected void onTick() 
	{
		...
		try 
		{
			...
			if (result.length > 0) 
			{
				...
				// 
				subscriber.addBehaviour(new CustomerAgentSubscrInitiator((CustomerAgent)subscriber, subscribe));
				//
	 ...
	 * Request message
	 */
	protected ACLMessage createRequestMessage(CustomerAgent sender, AID receiver) 
	{
		Date date = new Date();
