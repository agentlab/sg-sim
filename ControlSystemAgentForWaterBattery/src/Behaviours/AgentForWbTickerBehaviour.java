...
import Ontologies.AgentForWbOntology;
...
public class AgentForWbTickerBehaviour extends TickerBehaviour {
	...
	public AgentForWbTickerBehaviour(Agent a, ACLMessage msg) {
		super(a, 3000);
	    ...
	    if (planM.getPower().length<(step+1)) this.stop();
		  ...
		  try {
		   ...
		   for (AMSAgentDescription agent : agents) {
			  ...
			  if(agentID.getLocalName().compareTo("WB") != 0) continue;
			
			  aclmsg1.clearAllReceiver();
			  aclmsg1.addReceiver(agentID);
			  System.out.println(" ");
			  System.out.println("Current ActionMessage: " +planM.getPower()[step]+"  And his hour: "+step);
			  System.out.println("Sending "+step+" action to " + agentID);
			  step++;
			  myAgent.addBehaviour(new AgentForWBRequestInitiatorBehaviour(myAgent, aclmsg1));
		   }
		   }
	      }
	}
