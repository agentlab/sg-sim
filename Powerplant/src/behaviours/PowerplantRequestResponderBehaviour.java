...
public class PowerplantRequestResponderBehaviour extends AchieveREResponder {
	private static final long serialVersionUID = 1L;

	private PowerplantBehaviour powerplant;

	public PowerplantRequestResponderBehaviour(PowerplantBehaviour wt, Agent a, MessageTemplate mt) {
		super(a, mt);
		powerplant = wt;
	}
	...
	protected ACLMessage prepareResultNotification(ACLMessage msg, ACLMessage response) throws FailureException {
		...
		try {
			...
			if(action instanceof AssignPowerRequest) {
				...
				if(powerplant.setAssignedPower(apr.getAssignedPower())) {
			...
			if (action instanceof BeginPowerTransitionRequest) {
				...
				if (bptr.getAssignedPowerIsValid()) {
					...
					if(powerplant.setAssignedPower(bptr.getAssignedPower())) {
						System.out.println("Check passed, power assigned");
					} else {
						System.out.println("Out of bounds, power not assigned");
						isOk = false;
		...
		if(performed) {
			...
			td.setTime(powerplant.time_delay());
			System.out.println("Agent "+myAgent.getLocalName()+": Action successfully performed, time delay is " + powerplant.time_delay());
