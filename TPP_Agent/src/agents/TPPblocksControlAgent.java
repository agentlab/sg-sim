...
public class TPPblocksControlAgent extends Agent {

	/**
	 * 
	 */
	
	private static final long serialVersionUID = -8093256316291429406L;
	private Vector<TPPblockDescriptor> blocks = new Vector<TPPblockDescriptor>(); 		// параметры контролируемых блоков
	private Vector<Subscription> ccSubscription= new Vector<Subscription>();			// подписки блоков на команды управления
	private ControlCommandsSubscriptionManager ccSubMngr;								// SubMngr для команд управления
	private OneDayResultSubscriptionManager odrSubMngr;									// SubMngr для дневных результатов
	private Subscription odrSubscription;												// подписка произодителя на дневные результаты
	private TPPDescriptor myParams=new TPPDescriptor();									// дескриптор параметров TPP
	private PowerPlan curDayResult=new PowerPlan();														// произведено за день
	private PowerPlan nextDayPlan=new PowerPlan();														// план на следующий день
	...
	protected void setup(){
		...
		System.out.println("Hello, TPP blocks Control agent "+getAID().getLocalName()+" is started");
		myParams.setTppId(this.getAID());
		//register language and ontology
		this.getContentManager().registerLanguage(new SLCodec());
		this.getContentManager().registerOntology(TPPOntology.getInstance());
		this.getContentManager().registerOntology(PowerProducerOntology.getInstance());
		
		//Registering services on yellow pages
		ServiceDescription sd= new ServiceDescription();
		sd.setType("thermal-power-generator-control");
	...
	public void addBlock(TPPblockDescriptor tppBlock){
		this.blocks.add(tppBlock);
		return;
	}
	...
	public Vector<TPPblockDescriptor> getBlocks(){
		return this.blocks;
	}
	public void setBlocks(Vector<TPPblockDescriptor> list){
		this.blocks=list;
	}
	...
	public TPPDescriptor getMyParams() {
		return myParams;
	}

	public void setMyParams(TPPDescriptor myParams) {
	...
	public int getBlockIndex(AID blockID){
		int result=-1;
		for(TPPblockDescriptor d:this.blocks){
	...
	protected void takeDown(){
		...
		System.out.println("The TPP blocks control agent "+getAID().getLocalName()+" is terminated");
	}
}
